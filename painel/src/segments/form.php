<?php
    ...
    if($_POST['action'] == 'save'){
        ...
        if($_POST['id']){
            $query = "update segments set {$attr} where id = '{$_POST['id']}'";
            mysqli_query($con, $query);
            $id = $_POST['id'];
        }else{
            $query = "insert into segments set {$attr}";
            mysqli_query($con, $query);
    ...
    $query = "select * from segments where id = '{$_POST['id']}'";
...
<h4 class="Titulo<?=$md5?>"><?=$Dic['Segment Registration']?></h4>
    <form id="form-<?= $md5 ?>">
        <div class="row">
            <div class="col">
